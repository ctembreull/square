...
| **Production-as-source workflow** | Define sync strategy to prevent duplicates when prod data is promoted to repo. Problem: creating "white" style in prod (runtime), then adding identical "white" in dev creates duplicate on next deploy. Options: (1) `rake seeds:import` uses upsert with natural keys (name + team), (2) pre-deploy diff check warns of conflicts, (3) export task marks records with `exported_at` timestamp to track lineage. Affects: styles, colors, players. Must solve before football season or risk data corruption mid-event. |
