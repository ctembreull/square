...
| **Test admin email broadcast behavior** | Verify non-player admins receive emails when "Select All" is checked (broadcast mode) but NOT when sending to selective recipients. Check ActivityLog metadata tracks admin_recipients separately. Test with actual admin user accounts during pre-tournament setup. |
...
| ~~**Email sending page**~~ | ✅ Done - Dedicated email sender interface with 50/50 layout. Left: Post dropdown with live email preview (embedded HTML). Right: PDF status with live Turbo Stream updates, checkbox tree of families/singles with "Select All" control, admin broadcast mode (non-player admins included when all recipients selected), "Other Emails" field for ad-hoc addresses. PDF section shows badge status with tooltips, "Attach PDF" checkbox, regenerate button for stale PDFs, and warning text. Live updates via Turbo Streams show generating spinner, success/error states. ActivityLog tracks selective vs broadcast sends, player vs admin vs other recipients. |
...
| ~~**Job queue monitoring**~~ | ✅ Done - Self-healing via canary heartbeat. `QueueHeartbeatJob` writes to cache every 3 min (production only). `/status.json` returns 503 when heartbeat stale (>10 min), triggering Fly.io auto-restart. Cold start returns 200 with `"starting"` status to avoid false alarms during boot. `queue_stalled` ActivityLog warning for after-the-fact visibility (deduplicated to once per 30 min). |
...
| ~~**Litestream backups**~~ | ✅ Done - Continuous SQLite replication to Cloudflare R2 (10-second sync for main DB, 60-second for cache/queue). 7-day retention for main database, 1-day for ephemeral databases. Litestream runs as background process in production containers. |
...
| **Disaster recovery archive** | Create comprehensive USB drive archive for succession planning/disaster recovery. **Contents**: (1) All service credentials (Fly.io, Resend, Cloudflare R2, GitHub, domain registrar), (2) Complete code repository backup, (3) Database backups and seed data files, (4) Documentation (CLAUDE.md, README, deployment procedures), (5) Service account details and recovery codes, (6) Contact information and support links for all services. Clone and distribute to trusted family members. Not just runtime secrets, but everything needed to resurrect the app from scratch if catastrophic failure occurs. Update quarterly or after major infrastructure changes. |
...
Long-term speculative features for major future releases. These represent significant scope expansions and architectural decisions that go beyond incremental improvements.

| Item | Notes |
|------|-------|
| **Automated check mailing** | API integration with check printing/mailing services (Lob, Checkbook.io, Stripe Treasury) to programmatically generate and mail physical checks to winners. **Flow**: Admin approves payments → API call with recipient name/address/amount → Service prints and mails check → Database tracks as paid. **Cost**: ~$1-3 per check plus postage. **Benefits**: Automates admin burden while maintaining familiar check experience for recipients, avoids digital payment platform ToS issues, no payment credentials needed from players. **Use case**: Could enable estate planning scenarios (endowment/trust continuing the game long-term) by fully automating the payout process without requiring human intervention for each check. Prerequisites: Phase 1 payment tracking database (track payment status, method, dates). |
