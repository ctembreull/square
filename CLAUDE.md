...
| **Event completion timezone fix** | Bug: Events currently mark as completed at midnight UTC on end_date, causing premature completion (Super Bowl 2026 showed completed while game was still playing). Fix: Check event completion against midnight Hawaii time (UTC-10) instead of UTC. Ensures events stay active through end_date in westernmost US timezone. No Solomon Islands family members expected. |
...
| **Disaster recovery documentation** | Create `docs/DISASTER_RECOVERY.md` with step-by-step procedures: (1) Litestream point-in-time restore for fast recovery, (2) YAML rebuild for complete data loss, (3) Failure scenario playbooks (corrupted DB, lost volume, rollback needed), (4) Monthly drill checklist. Include both Fly.io restore and local restore procedures. Test restore process and document actual timings. |
