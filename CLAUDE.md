...
| **Production-as-source workflow** | Daily R2 sync makes prod the canonical source. **Push (prod→R2)**: Daily job runs `seeds:export`, `affiliations:export`, `players:export`, then uploads YAML files to R2 bucket (no versioning, overwrite). **Pull (R2→dev)**: `rake r2:pull` downloads YAMLs to `db/seeds/`, then run existing import tasks. Uses `aws-sdk-s3` gem (R2 is S3-compatible). Same bucket as Litestream. Workflow: always pull before modifying team data locally. |
